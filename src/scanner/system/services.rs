...
pub fn generate_timeline(parser: &mut Parser, target: &String, controlset: u32, noisy: bool) ->  Option<String> {
                ...
                Some(mut key) => {
                    for skey in key.read_sub_keys(parser) {
                        ...
                        if noisy || servicetype.contains("Win32OwnProcess") {
                            results.push(format!("services\tService \"{}\" has been modified\t{}\t{}\\{}\t{}", skey.key_name, target, key_path, skey.key_name, last_key_write_timestamp));
                        }
                    }
...
pub fn get_asep(parser: &mut Parser, target: &String, controlset: u32, noisy: bool) ->  Option<String> {
    let mut results: Vec<String> = Vec::new();
    let key_path = format!("ControlSet00{}\\Services", controlset);
