...
pub fn generate_timeline(parser: &mut Parser, target: &String) ->  Option<String> {
    let key_path = "Software\\Microsoft\\Windows\\CurrentVersion\\Group Policy\\Scripts\\Logon";
                ...
                Some(mut key) => {
                    ...
                    for mut skey in sub_keys {
                        ...
                        for skey2 in sub_keys2 {
                            ...
                            results.push(format!("logonscripts\tLogon Script of \"{}\" has been configured (Parameters: \"{}\")\t{}\t{}\t{}", script, parameters, target, skey2.path, last_key_write_timestamp));
                        }
                    }
                ...
                Some(mut key) => {
                    ...
                    for mut skey in sub_keys {
                        ...
                        for skey2 in sub_keys2 {
                            ...
                            results.push(format!("logonscripts\t{}\t{}\t{}\t{}\t{}\tParameters: \"{}\"", script, target, skey2.path, "*", last_key_write_timestamp, parameters));
                        }
                    }
