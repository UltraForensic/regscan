...
pub fn scan(parser: &mut Parser, target: &String) ->  Option<String> {
    let key_path = "Microsoft\\Windows\\CurrentVersion\\Group Policy\\History";
                ...
                Some(mut key) => {
                    ...
                    for mut skey in sub_keys {
                        ...
                        for skey2 in sub_keys2 {
                            ...
                            results.push(format!("software_gpo_history\tGPO history has been found (DisplayName: \"{}\", FileSysPath: \"{}\", Link: \"{}\")\t{}\t{}\t{}", displayname, filesyspath, link, target, key_path, last_key_write_timestamp));
                        }
                    }
